module Test.ManualSet exposing (tests)
...
singletonTests =
    ...
    , test "contains given element" <|
        \() -> Expect.equal True (Set.member identity 1 (Set.singleton identity 1))
    ]
...
insertTests =
    [ test "adds new element to empty set" <|
        \() -> Expect.equal set42 (Set.insert identity 42 Set.empty)
    , test "adds new element to a set of 100" <|
        \() -> Expect.equal (Set.fromList identity (List.range 1 101)) (Set.insert identity 101 set1To100)
    , test "leaves singleton set intact if it contains given element" <|
        \() -> Expect.equal set42 (Set.insert identity 42 set42)
    , test "leaves set of 100 intact if it contains given element" <|
        \() -> Expect.equal set1To100 (Set.insert identity 42 set1To100)
    ]
...
removeTests =
    [ test "removes element from singleton set" <|
        \() -> Expect.equal Set.empty (Set.remove identity 42 set42)
    , test "removes element from set of 100" <|
        \() -> Expect.equal (Set.fromList identity (List.range 1 99)) (Set.remove identity 100 set1To100)
    , test "leaves singleton set intact if it doesn't contain given element" <|
        \() -> Expect.equal set42 (Set.remove identity -1 set42)
    , test "leaves set of 100 intact if it doesn't contain given element" <|
        \() -> Expect.equal set1To100 (Set.remove identity -1 set1To100)
    ]
...
memberTests =
    [ test "returns True when given element inside singleton set" <|
        \() -> Expect.equal True (Set.member identity 42 set42)
    , test "returns True when given element inside set of 100" <|
        \() -> Expect.equal True (Set.member identity 42 set1To100)
    , test "returns False for element not in singleton" <|
        \() -> Expect.equal False (Set.member identity -1 set42)
    , test "returns False for element not in set of 100" <|
        \() -> Expect.equal False (Set.member identity -1 set1To100)
    ]
...
foldlTests =
    [ test "with insert and empty set acts as identity function" <|
        \() -> Expect.equal set1To100 (Set.foldl (Set.insert identity) Set.empty set1To100)
...
foldrTests =
    [ test "with insert and empty set acts as identity function" <|
        \() -> Expect.equal set1To100 (Set.foldr (Set.insert identity) Set.empty set1To100)
    , test "with counter and zero acts as size function" <|
...
unionTests =
    [ test "with empty set doesn't change anything" <|
        \() -> Expect.equal set42 (Set.union identity set42 Set.empty)
    , test "with itself doesn't change anything" <|
        \() -> Expect.equal set1To100 (Set.union identity set1To100 set1To100)
    , test "with subset doesn't change anything" <|
        \() -> Expect.equal set1To100 (Set.union identity set1To100 set42)
    , test "with superset returns superset" <|
        \() -> Expect.equal set1To100 (Set.union identity set42 set1To100)
    , test "contains elements of both singletons" <|
        \() -> Expect.equal (Set.insert identity 1 set42) (Set.union identity set42 (Set.singleton identity 1))
    , test "consists of elements from either set" <|
        \() ->
            Set.union identity set1To100 set51To150
...
intersectTests =
    [ test "with empty set returns empty set" <|
        \() -> Expect.equal Set.empty (Set.intersect identity set42 Set.empty)
    , test "with itself doesn't change anything" <|
        \() -> Expect.equal set1To100 (Set.intersect identity set1To100 set1To100)
    , test "with subset returns subset" <|
        \() -> Expect.equal set42 (Set.intersect identity set1To100 set42)
    , test "with superset doesn't change anything" <|
        \() -> Expect.equal set42 (Set.intersect identity set42 set1To100)
    , test "returns empty set given disjunctive sets" <|
        \() -> Expect.equal Set.empty (Set.intersect identity set42 (Set.singleton identity 1))
    , test "consists of common elements only" <|
        \() ->
            Set.intersect identity set1To100 set51To150
...
diffTests =
    [ test "with empty set doesn't change anything" <|
        \() -> Expect.equal set42 (Set.diff identity set42 Set.empty)
    , test "with itself returns empty set" <|
        \() -> Expect.equal Set.empty (Set.diff identity set1To100 set1To100)
    , test "with subset returns set without subset elements" <|
        \() -> Expect.equal (Set.remove identity 42 set1To100) (Set.diff identity set1To100 set42)
    , test "with superset returns empty set" <|
        \() -> Expect.equal Set.empty (Set.diff identity set42 set1To100)
    , test "doesn't change anything given disjunctive sets" <|
        \() -> Expect.equal set42 (Set.diff identity set42 (Set.singleton identity 1))
    , test "only keeps values that don't appear in the second set" <|
        \() ->
            Set.diff identity set1To100 set51To150
...
fromListTests =
    [ test "returns empty set for empty list" <|
        \() -> Expect.equal Set.empty (Set.fromList identity [])
...
reHashTest = 
    [ describe "if rehashing leads to collisions the key with the highes value under current hashing wins"
        [ test "some ordering in both hashinngs" <|
            \() -> Expect.equal
                (Set.fromList identity [-1,1] |> Set.reHash abs |> Set.toList)
                [1]
        , test "inver ordering in new hashing" <|
            \() -> Expect.equal
                (Set.fromList identity [-1,1] |> Set.reHash (\n -> -(abs n)) |> Set.toList)
                [1]
        ]
    ]
...
eqTest = 
    [ test "some" <|
        \() -> 
            ...
            in
                Expect.equal
                    (Set.eq String.fromInt identity set1 set2)
                    True
    ]
